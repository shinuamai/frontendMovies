...
export const Recommendations = () => {
  ...
  const [recommendationToEdit, setRecommendationToEdit] =
    ...
    const loadRecommendations = async () => {
      try {
        ...
        const userRecommendations =
          await recommendationBackendService.getByUser(1);
  ...
  const handleToggleViewed = async (id: number, currentStatus: boolean) => {
    ...
    try {
      ...
      console.log(
        `✅ Recommendation ${id} status updated to ${
          newStatus ? "viewed" : "pending"
        }`
      );
      setError(null);
    } catch (err: any) {
      ...
      setError(
        `Error al cambiar estado: ${err.message || "Error desconocido"}`
      );
    } finally {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="text-sm text-gray-500">
          ...
          <div>
            Vistas: {recommendations.filter((r) => r.visualizada).length}
          </div>
          <div>
            Pendientes: {recommendations.filter((r) => !r.visualizada).length}
          </div>
        </div>
      </div>
